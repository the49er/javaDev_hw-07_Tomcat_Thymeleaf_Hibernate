...
import com.goit.javadev.database.model.CrudEntityDaoJDBC;
...
@Slf4j
public class CompanyDaoJDBC implements CrudEntityDaoJDBC<Company> {
    private static final String TABLE_NAME = "`homework_7`.";
    ...
    public CompanyDaoJDBC(Connection connection) {
        try {
            insertSt = connection.prepareStatement(
                    "INSERT INTO "+ TABLE_NAME + "companies (name, specialization) VALUES (?, ?)"
            );
            ...
            clearTableSt = connection.prepareStatement(
                    "DELETE FROM " + TABLE_NAME + "companies"
            );

            deleteById = connection.prepareStatement(
                    "DELETE FROM "+ TABLE_NAME + "companies WHERE id = ?"
            );
    ...
    public boolean updateEntityFieldsById(Company element, long id) {
        try {
            ...
            log.info("CompanyJDBC with id: " + id + " has been updated");
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
